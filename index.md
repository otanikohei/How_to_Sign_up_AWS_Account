...
AWS アカウント内で実行された API アクションを記録するサービスです。
例えば EC2 インスタンスを削除した場合だと
...
CloudTrail は有事の際に不正アクセスかどうか判断する材料になります。
...
AWS サービスは「マネージメントコンソール」、「AWS CLI」、「SDK」、「他AWSサービス」から操作ができますが、ほとんどのサービスは API エンドポイントへアクセスします。
CloudTrail はこの API エンドポイントのアクセス記録を記録するサービスです。
